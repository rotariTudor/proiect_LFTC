...
Instr *genTestProgram2(){
	...
	addInstrWithInt(&code,OP_FPSTORE,1);
	// while(i<n){
	Instr *whilePos=addInstrWithInt(&code,OP_FPLOAD,1);	// push i
	addInstrWithInt(&code,OP_FPLOAD,-2);				// push n
	addInstr(&code,OP_LESS_F);							// i<n -> int pe stiva
	Instr *jfAfter=addInstr(&code,OP_JF);
	// put_d(i);
	addInstrWithInt(&code,OP_FPLOAD,1);					// push i
	...
	addInstrWithInt(&code,OP_FPSTORE,1);				// i = rezultat
	// } (next iteration)
	addInstr(&code,OP_JMP)->arg.instr=whilePos;
